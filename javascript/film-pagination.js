...
//////////////////////////////////////////////////////////////////////////////////
let serialsPagination = document.querySelector(".films-pagination");
let serialsInPlayingBtn = document.querySelector(".films-in-playing-btn");
...
let showSerialPaginationFlex = document.querySelectorAll('.show-film-pagination-flex')
let newSerialsArray;
let numberPage;
let HmSerial = 0
let SerialPaginationFlex = document.querySelector("#showFilmPaginationFlex")
fetch('https://filmnet-b6d8b-default-rtdb.firebaseio.com/film.json')
  ...
  .then(data => {
    ...
    let serialPaginationNumber = document.querySelector('.film-pagination-number')
    for (let f = 0; f < HmNumberPage; f++) {
      serialPaginationNumber.insertAdjacentHTML("beforeend", `<div>${(f + 1)}</div>`)
    }
    let allNumberPage = document.querySelectorAll('.film-pagination-number div')
    serialFetch.slice(-HmShowSerial,).forEach(item => {
      ...
      <div class="film-pagination" style="background: url('${item[1].image}');">
      <div class="hide-cover-film videos film-in-playing">${item[1].description}</div>
      <h2 class="film-pagination-name">${item[1].name}</h2>
      <p class="film-score"><span>${item[1].score}</span>/10</p>
      </div></a>`)
    })
    allNumberPage.forEach(items => {
      items.addEventListener('click', item => {
        ...
        let SerialPaginationFlex = document.querySelector(".show-film-pagination-flex")
